...
export const HoverEffect = ({ items, className }) => {
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-x-8 gap-y-2 py-5", 
        className
      )}
    >
      {items.map((item, idx) => (
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 bg-neutral-200 dark:bg-customGray/[0.8] block rounded-3xl"
...
export const CardTitle = ({ className, children }) => {
  return (
    <a className={cn("text-customGray font-bold tracking-wide text-lg", className)}> 
      {children}
    </a>
  );
};
